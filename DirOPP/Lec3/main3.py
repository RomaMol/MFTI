...
class House:
    ...
    def __getattribute__(self, item):
        """
        Перегруженный метод
        проверка (h1._House__x) --> _House__x
        :param item:
        :return:
        """
        if item == "_House__x":
    ...
    def __setattr__(self, key, value):
        """ Перегруженный метод при проверки статической пееременной"""
        if key == "STAT":
    ...
    def __getattr__(self, item):
        """вызывается при отсутствие атрибута"""
        print("__getattr__" + item)

    def __delattr__(self, item):
        """вызывается при удаления атрибута"""
